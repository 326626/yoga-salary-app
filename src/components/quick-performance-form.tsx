  ...
  return (
    <div className="space-y-5">
      ...
      <section className="space-y-3">
        ...
        {visibleRecords.length === 0 ? <EmptyState text="还没有业绩记录，记录第一笔收入吧～" /> : visibleRecords.slice(0, 12).map((record) => <PerformanceCard key={record.id} record={record} teachers={teachers} studios={studios} members={members} packages={packages} onEdit={startEdit} onDelete={removeRecord} />)}
      </section>
    </div>
  );
}

function PerformanceCard({ record, teachers, studios, members, packages, onEdit, onDelete }: { record: Performance; teachers: Teacher[]; studios: Studio[]; members: Member[]; packages: MemberPackage[]; onEdit: (record: Performance) => void; onDelete: (record: Performance) => void }) {
...
function friendlyError(field: keyof FieldErrors, fallback: string) {
  const messages: FieldErrors = { teacher_id: "请选择当前老师", studio_id: "请选择瑜伽馆", date: "请选择日期", type: "请选择业绩类型", amount: "金额不能小于 0" };
